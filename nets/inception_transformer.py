...
from .basic_blocks import DecoderBlock
...
class InceptionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=None, depths=None,
        ...
        # self.ppm = PyramidPooling(embed_dims[3], embed_dims[3])
        # self.d4 = DecoderBlock(embed_dims[3], embed_dims[2])
        # self.d3 = DecoderBlock(embed_dims[2], embed_dims[1])
        # self.d2 = DecoderBlock(embed_dims[1], embed_dims[0])
        # self.d1 = DecoderBlock(embed_dims[0], embed_dims[0])

        # self.output = nn.Sequential(
        #     nn.Conv2d(embed_dims[0], embed_dims[0] // 2, 1),
        #     nn.ReLU(inplace=True),
        #     nn.ConvTranspose2d(
        #         embed_dims[0] // 2, embed_dims[0] // 2, 3, stride=2, padding=1, output_padding=1
        #     ),
        #     nn.Conv2d(embed_dims[0] // 2, 1, 1)
        # )
    ...
    def forward_features(self, inputs):
        ...
        # x_e4 = self.ppm(x_e4)
        #
        # x_d4 = self.d4(x_e4) + x_e3
        # x_d3 = self.d3(x_d4) + x_e2
        # x_d2 = self.d2(x_d3) + x_e1
        # x_d1 = self.d1(x_d2)
        
        # output = self.output(x_d1)
        return torch.sigmoid(x_e4)
...
def iformer_small_384(pretrained=False, **kwargs):
    ...
    if pretrained:
        path = '../model_weight/iformer_small_384.pth'
